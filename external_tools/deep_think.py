"""Route hard questions to the strongest available model and speak the answer.
...
Which model answers is the "깊은 판단" role in the admin panel, not a constant
here. The tool is named for what it does rather than for who does it, so that
the robot never tells the user it asked one company while asking another.
...
from _secretary_lib.reason import ReasonError, complete, provider_label  # noqa: E402

logger = logging.getLogger(__name__)

MAX_TOKENS = 8000

# The answer is spoken, not read. Everything here exists to stop the model from
...
def _memory_context(instance_path: Any) -> str:
    ...
    try:
        ...
        logger.debug("Could not load memory facts for deep_think: %s", exc)
        return ""


class DeepThink(Tool):
    """Delegate a reasoning-heavy question to the strongest configured model."""

    name = "deep_think"
    description = (
        "Hand something hard to a much stronger reasoning model and get an answer back. "
        ...
        "properties": {
            "question": {
                ...
                "description": (
                    ...
                    "It cannot see the conversation, so do not use pronouns like '그거' or '아까 그것' - "
            ...
            "context": {
                "type": "string",
                "description": (
                    "Anything from the conversation it needs in order to answer: what the user already "
    ...
    async def __call__(self, deps: ToolDependencies, **kwargs: Any) -> dict[str, Any]:
        """Send the question to the judgement model and return the spoken answer."""
        ...
        logger.info("Tool call: deep_think question=%s", question[:120])
        ...
        try:
            ...
            # Providers that have no effort dial ignore it.
        ...
        # Report who actually answered. A hardcoded name here becomes a lie the
        # moment the role is pointed at a different provider.
        return {"answer": answer, "model": provider_label()}
